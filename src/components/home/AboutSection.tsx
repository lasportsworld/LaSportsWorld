      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-4 sm:px-6 lg:grid-cols-[.9fr_1fr_.55fr] lg:gap-14 lg:px-8">
        <div className="relative hidden h-[360px] lg:block">
          <div className="absolute left-6 top-0 h-56 w-80 rotate-[-2deg] overflow-hidden rounded border-[7px] border-white shadow-xl">
            <Image src="/images/lasw_slideshow_001.jpg" alt="Coach talking with campers" fill className="object-cover" />
          </div>
          <div className="absolute bottom-4 right-2 h-44 w-56 rotate-[3deg] overflow-hidden rounded border-[7px] border-white shadow-xl">
            <Image src="/images/lasw_slideshow_003.jpg" alt="Coach leading sports drills" fill className="object-cover" />
          </div>
        </div>

        <div>
          <p className="mb-3 text-xs font-extrabold uppercase tracking-[0.2em] text-gold">About LA Sports World</p>
          <h2
          ...
          <p className="mt-5 max-w-2xl text-sm leading-7 text-navy/65">
            At LA Sports World, we combine skill-building, character development, and good old-fashioned fun in a
            safe and supportive environment. Our programs are led by experienced coaches who care about every
            child&apos;s growth on and off the field.
          </p>
          <Link
            href="/about"
            className="mt-7 inline-flex items-center gap-3 rounded-lg bg-navy px-7 py-3.5 text-xs font-extrabold uppercase tracking-wide text-white shadow-lg shadow-navy/15 transition hover:bg-navy-light"
          >
        ...
        <div className="grid gap-0 divide-y divide-navy/15 border-y border-navy/15 lg:border-l lg:border-y-0 lg:pl-8">
          {features.map(({ Icon, label }) => (
            <div key={label} className="flex items-center gap-4 py-5">
              ...
              <p className="text-sm font-extrabold leading-tight text-navy">{label}</p>
            </div>
          ))}
        </div>
      </div>
