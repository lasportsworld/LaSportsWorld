...
export default function OrganizationInquiryForm({ initialProgramType = "", initialEngagementType = "", entryContext = "schools-businesses-overview", sourcePage = "/schools-businesses/request" }: { initialProgramType?: OrganizationProgramType | ""; initialEngagementType?: EngagementType | ""; entryContext?: string; sourcePage?: string }) {
  ...
  if (state.status === "success") return <InquirySuccess focusRef={successRef} title="Your request is with us." description="Thanks for sharing the shape of your program or event. We’ll review the details and follow up with the most useful next questions." primary={{ label: "Back to Schools & Businesses", href: "/schools-businesses" }} secondary={{ label: "Return Home", href: "/" }} />;
