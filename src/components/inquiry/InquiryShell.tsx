...
export default function InquiryShell({
  ...
  highlights,
  children,
  note = "No commitment. Just enough context for a thoughtful recommendation.",
  variant = "coaching",
}: {
  eyebrow: string;
  title: string;
  ...
  children: ReactNode;
  note?: string;
  variant?: "coaching" | "party" | "organization";
}) {
  const accent = variant === "party" ? "bg-gold/25" : variant === "organization" ? "bg-white/10" : "bg-gold/20";
  return (
